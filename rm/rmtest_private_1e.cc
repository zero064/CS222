...
int TEST_RM_PRIVATE_1E(const string &tableName)
{
    // Functions tested
    // 1. Scan Table (VarChar with Nulls)
    cout << "***** In TEST_RM_PRIVATE_1E *****" << endl;
    ...
    if(rc != success) {
        free(returnedData);
        cout << "***** TEST_RM_PRIVATE_1E failed *****" << endl << endl;
        return -1;
    }
    ...
    while(rmsi2.getNextTuple(rid, returnedData) != RM_EOF)
    {
		...
		// There are 50 tuples whose message_text value is NULL
    	// For these tuples, send_time is also NULL. These NULLs should not be returned.
		nullBit = *(unsigned char *)((char *)returnedData) & (1 << 6);
		
		if (nullBit) {
             cout << "***** A wrong Entry was returned. TEST_RM_PRIVATE_1E failed *****" << endl << endl;
             rmsi2.close();
             free(returnedData);
             free(value);
             return -1;
    ...
    if (counter != 949){
       cout << "***** The number of returned tuple: " << counter << " is not correct. TEST_RM_PRIVATE_1E Failed *****" << endl << endl;
    } else {
       cout << "***** TEST_RM_PRIVATE_1E Passed *****" << endl << endl; 
    }
    return 0;
}
...
int main()
{
    ...
    RC rcmain = TEST_RM_PRIVATE_1E("tbl_private_1b");
    return rcmain;
}
